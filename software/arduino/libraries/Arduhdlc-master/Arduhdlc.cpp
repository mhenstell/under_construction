...
void Arduhdlc::receiveArray(uint8_t *framebuffer, uint8_t frame_length)
{   
    uint8_t data;
    while (frame_length)
    {   
...
void Arduhdlc::receive(uint8_t data)
{
    /* FRAME FLAG */
    if(data == FRAME_BOUNDARY_OCTET)
    {
        ...
        /* If a valid frame is detected */
                  ...
                  ( this->frame_checksum == ((this->receive_frame_buffer[this->frame_position-2] << 8 ) | (this->receive_frame_buffer[this->frame_position-1] & 0xff)) ) )  // (msb << 8 ) | (lsb & 0xff)
        {
...
void Arduhdlc::send(const char *framebuffer, uint8_t frame_length)
{
    uint8_t data;
...
uint8_t Arduhdlc::makeBuffer(const uint8_t *framebuffer, uint8_t *output, uint8_t frame_length)
{
    uint8_t data;
    uint16_t fcs = CRC16_CCITT_INIT_VAL;

    uint8_t outIdx = 0;
    output[outIdx++] = FRAME_BOUNDARY_OCTET;
    ...
    while(frame_length)
    {
        ...
        if((data == CONTROL_ESCAPE_OCTET) || (data == FRAME_BOUNDARY_OCTET))
        {
            output[outIdx++] = CONTROL_ESCAPE_OCTET;
            data ^= INVERT_OCTET;
        }
        output[outIdx++] = data;
        frame_length--;
